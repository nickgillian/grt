...
bool train_mlp_xor(const bool enableScaling) {
  ...
  //Initialize the MLP
  EXPECT_TRUE( mlp.init(numInputNeurons, numHiddenNeurons, numOutputNeurons, Neuron::LINEAR, Neuron::TANH, Neuron::TANH ) );
  ...
  EXPECT_TRUE( mlp.setLearningRate( 0.2 ) ); //This sets the rate at which the learning algorithm updates the weights of the neural network
  EXPECT_TRUE( mlp.setMomentum( 0.5 ) ); //This sets the amount of smoothing between weight updates
  EXPECT_TRUE( mlp.setNumRandomTrainingIterations( 1 ) ); //This sets the number of times the MLP will be trained, we manually do the re-try training below as needed
